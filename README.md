...
| ---------------------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `pull-request.yaml`          | Any PR except PRs to `main` · push `develop` · manual         | `flutter analyze` + `flutter test --coverage`, scope lcov to the activated surface, fail below the committed floor, upload lcov artifact. Jobs: `Analyze & Test`, `Coverage Floor Gate`, `BitBox quirks audit`. |
