...
- **Remaining KYC-step cubits without a spec** — `kyc/steps/ident/cubits` only (the Sumsub-SDK wrapper listed under "Surface that needs infra work" in `docs/testing.md`). `kyc/steps/2fa/cubits` and `kyc/steps/nationality/cubit` are covered at 100% line coverage by `test/screens/kyc/steps/2fa/*_cubit_test.dart` and `test/screens/kyc/steps/nationality/kyc_nationality_cubit_test.dart`.
