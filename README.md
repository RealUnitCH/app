...
The Android `beta` lane pushes the **Play Store listing** (Fastlane metadata + screenshots) alongside the AAB on every tag-driven release, so the Play listing stays in sync with the build (production promotion / final submit stay manual — see `store-metadata.yaml`). The **App Store listing is not** coupled to the iOS `beta` lane: `deliver` can only write to an editable App Store version, so the iOS listing is driven separately via the `store_metadata` lane / `store-metadata.yaml` once such a version exists — the iOS `beta` lane only ships the TestFlight binary. Either way, `release.yaml` runs the same `scripts/check-store-metadata.sh` preflight (FIXME placeholders + character/URL limits) as `store-metadata.yaml` in a gating `store-metadata-preflight` job before either deploy lane runs — a tag can never ship a `FIXME-` placeholder or an oversize field to the live consoles.
