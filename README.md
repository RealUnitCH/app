...
| ---------------------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `pull-request.yaml`          | Any PR except PRs to `main` · push `develop` · push `staging` fail-safe · manual | `Route CI execution` selects one authoritative staging lane: the ready `staging → develop` PR normally owns the canonical required checks, while the staging-push suite runs only if that PR is unavailable. Heavy jobs run `flutter analyze` + `flutter test --coverage --exclude-tags golden`, enforce scoped coverage, validate Goldens on the self-hosted runner, and upload diagnostics. Push and PR concurrency remain separate so deduplication cannot leave cancelled checks on the current PR head. |
