...
bool isSeedQr(String seedQRBytes) => seedQRBytes.length == 48 || seedQRBytes.length == 96;

bool isCompactSeedQr(List<int> rawByteData) =>
    hex.encode(rawByteData).startsWith('410') || hex.encode(rawByteData).startsWith('420');
...
String getSeedFromCompactSeedQr(List<int> rawByteData) {
  ...
  final checksum =
      hex.encode(sha256.convert(hex.decode(hexShort)).bytes).substring(0, checksumLimit);

  final seedQRBytes =
      BigInt.parse("$hexShort$checksum", radix: 16).toRadixString(2).padLeft(binaryLength, "0");

  final indexes =
      RegExp(r'.{1,11}').allMatches(seedQRBytes).map((e) => int.parse(e.group(0)!, radix: 2));
  return indexes.map((e) => wordlist.WORDLIST[e]).join(" ");
}
