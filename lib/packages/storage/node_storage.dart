...
extension NodeStorage on AppDatabase {
  Future<int> insertNode(int chainId, String name, String httpsUrl, String? wssUrl) =>
  ...
  Future<int> updateNode(int chainId, {String? httpsUrl, String? wssUrl}) =>
      (update(nodes)..where((row) => row.chainId.equals(chainId))).write(NodesCompanion(
        httpsUrl: Value.absentIfNull(httpsUrl),
        wssUrl: Value.absentIfNull(wssUrl),
      ));

  Future<NodeData?> getNode(int chainId) =>
      (select(nodes)..where((row) => row.chainId.equals(chainId))).getSingleOrNull();
