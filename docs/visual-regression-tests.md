...
The 308 PNGs the handbook serves at `handbook.realunit.app/screenshots/`
...
4. Open the PR. The `Handbook Build Check` workflow runs
   ...
   spot-checks selected mapped screenshots via `docker exec test -f`; the full set is gated by the assemble step (`expected 308`)). A missing Golden surfaces here
