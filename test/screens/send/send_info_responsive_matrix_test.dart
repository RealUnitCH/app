...
void main() {
  ...
  group('SendInfoPage responsive matrix (full device × textScale)', () {
    for (final cell in kFullResponsiveMatrix) {
      testWidgets(cell.id, (tester) async {
        await withTargetPlatform(cell.device.platform, () async {
          await expectNoLayoutOverflow(
            tester,
            () => _pumpScreen(tester, cell, const SendInfoPage()),
            reason: 'SendInfoPage overflow / ${cell.label}',
          );

          await expectFullyTappable(
            tester,
            find.widgetWithText(FilledButton, S.current.next),
            within: find.byType(SendInfoPage),
            reason: 'SendInfoPage / ${cell.label}: Continue CTA not tappable',
          );
        });
      });
    }
