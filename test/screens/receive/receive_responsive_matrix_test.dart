...
void main() {
  ...
  group('ReceivePage responsive matrix (bottom sheet)', () {
      ...
      testWidgets(cell.id, (tester) async {
        await withTargetPlatform(cell.device.platform, () async {
          await expectNoLayoutOverflow(
            ...
            reason: 'ReceivePage (bottom sheet) overflow / ${cell.label}',
          );

          await expectFullyTappable(
            tester,
            find.widgetWithText(AppFilledButton, S.current.send),
            within: find.byType(ReceivePage),
            reason:
                'ReceivePage (bottom sheet) / ${cell.label}: Send CTA not tappable',
          );
        });
      });
    }
  ...
  group('ReceivePage responsive matrix (full page)', () {
    for (final cell in kFullResponsiveMatrix) {
      testWidgets(cell.id, (tester) async {
        await withTargetPlatform(cell.device.platform, () async {
          await expectNoLayoutOverflow(
            ...
            () => _pumpScreen(
              tester,
              cell,
              const ReceivePage(isBottomSheet: false),
            ),
            reason: 'ReceivePage (full page) overflow / ${cell.label}',
          );
          ...
          await expectFullyTappable(
            ...
            within: find.byType(ReceivePage),
            reason:
                'ReceivePage (full page) / ${cell.label}: Send CTA not tappable',
          );
        });
      });
    }
