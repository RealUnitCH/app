...
void main() {
  ...
  group('$SellPaymentInfoCubit', () {
    ...
    test('BitboxNotConnectedException → Failure(bitboxDisconnected)', () async {
      // The cubit re-shapes the typed BitBox exception (which can surface
      // from the EIP-7702 sign path inside getPaymentInfo when the device
      // dropped its BLE link) into a UI-routable failure state with the
      // bitboxDisconnected error code — every other catch arm carries a
      // different error code, so the test pins the routing.
      when(
        () => service.getPaymentInfo(any(), any(), currency: any(named: 'currency')),
      ...
      expect(f.message, contains('BitBox'));
    });
