...
void main() {
  ...
  group('$CreateWalletCubit', () {
    ...
    test('createWallet stores the newly generated draft in state', () async {
      ...
      when(() => service.generateUncommittedSeedDraft(any()))
      ...
      verify(() => service.generateUncommittedSeedDraft('Obi-Wallet-Kenobi'))
          .called(1);
    ...
    group('app lifecycle', () {
      testWidgets('hidden drops the just-generated mnemonic from cubit state',
        ...
        // not the seed.
        expect(initialDraft.isDisposed, isTrue,
            reason: 'BL-018: hidden must dispose the draft, not just '
                'drop the cubit reference');
        expect(emissions, isNotEmpty,
            reason: 'hidden must emit at least the cleared state');
        expect(emissions.first.draft, isNull,
      ...
      testWidgets('hidden is a no-op when no draft has been generated yet',
          (tester) async {
        final cubit = CreateWalletCubit(service, authService);
        addTearDown(cubit.close);
      ...
      testWidgets(
          'hidden -> resumed re-generates a fresh draft so the view is not '
        ...
        expect(emissions.last.draft, isNotNull,
            reason: 'fresh draft must replace the cleared state');
        expect(emissions.last.draft, isNot(same(initial)),
            reason: 'a NEW SeedDraft must be generated, not the cleared one');
        expect(generated, 2,
            reason: '_dropMnemonic must re-fire generateUncommittedSeedDraft '
