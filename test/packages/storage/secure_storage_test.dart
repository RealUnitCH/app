...
void main() {
  ...
  group('SecureStorage verifyPin', () {
    ...
    test('legacy 100k hash is accepted once and transparently rehashed', () async {
      final salt = SecureStorage.generatePinSalt();
      final legacyHash = SecureStorage.hashPin('123456', salt, iterations: 100000);
    ...
    test('10k hash is rejected even when the PIN is correct', () async {
      final salt = SecureStorage.generatePinSalt();
      final rejectedHash = SecureStorage.hashPin('123456', salt, iterations: 10000);

      when(
        () => mockStorage.read(key: 'pin.hash'),
      ).thenAnswer((_) async => rejectedHash);
      when(
        () => mockStorage.read(key: 'pin.salt'),
      ).thenAnswer((_) async => bytesToHex(salt));
  ...
  group('SecureStorage mnemonic key cleanup', () {
  ...
  group('SecureStorage biometric crypto sentinel API', () {
    test('readBiometricCryptoSentinel forwards the provided key', () async {
      when(
        () => mockStorage.read(key: 'biometric.cryptoObject.sentinel'),
      ).thenAnswer((_) async => 'sentinel');

      expect(
        await secureStorage.readBiometricCryptoSentinel('biometric.cryptoObject.sentinel'),
        'sentinel',
      );
    });

    test('writeBiometricCryptoSentinel forwards the provided key and value', () async {
      await secureStorage.writeBiometricCryptoSentinel(
        'biometric.cryptoObject.sentinel',
        'secret',
      );

      verify(
        () => mockStorage.write(
          key: 'biometric.cryptoObject.sentinel',
          value: 'secret',
        ),
      ).called(1);
    });
